...
pub fn get_tags_from_filename(filename: &str) -> Option<(String, String)> {
    ...
    let trimmed_filename = filename.trim_start_matches("Various Artists - ").trim();
    if let Some((artist_part, title_part)) = trimmed_filename.split_once(" - ") {
        let artist = normalize_str(artist_part.trim());
        let title = normalize_str(title_part.trim());
        Some((artist, title))
    } else {
        None
    }
...
mod test_get_tags_from_filename {
    ...
    #[test]
    fn returns_empty_artist_for_no_delimiter() {
    ...
    #[test]
    fn preserves_additional_delimiters_in_title() {
    ...
    #[test]
    fn strips_various_artists_prefix() {
        let filename = "Various Artists - DJ Snake - Turn Down for What";
        assert_eq!(
            get_tags_from_filename(filename),
